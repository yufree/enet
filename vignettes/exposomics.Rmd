...
Here we can find most of the metabolites are associated with multiple exposures and 22 of those metabolites can be defined as gatekeepers.

## Check association between gatekeepers and health outcome

```{r}
metagk <- meta[match(rownames(re$me),rownames(meta)),]
re2 <- genefilter::rowttests(metagk,factor(asthma))
re2
```

Here we can find three gatekeepers might mediate the influnces from multiple exposures to asthma condition.
